...
/*   Updated: 2023/04/04 16:01:53 by vcodrean         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (game->matrix[i][j].value_aux == '0' || \
	game->matrix[i][j].value_aux == 'C')
...
int	complete_path(t_game *game, int x, int y)
{
	...
	while (i < x)
	{
		...
		while (j < y)
		{
			if ((game->matrix[i][j].value_aux == 'E' && \
			game->matrix[i + 1][j].value_aux == 'P')
			|| (game->matrix[i][j].value_aux == 'E' && \
			game->matrix[i - 1][j].value_aux == 'P')
			|| (game->matrix[i][j].value_aux == 'E' && \
			game->matrix[i][j + 1].value_aux == 'P')
			|| (game->matrix[i][j].value_aux == 'E' && \
			game->matrix[i][j - 1].value_aux == 'P'))
				return (1);
			j++;
